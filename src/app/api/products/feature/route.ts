...
export async function POST(req: Request) {
  try {
    ...
    // Rate limiting: máximo 10 productos destacados por hora
    const rateLimitResult = await checkRateLimit({
      identifier: user.id,
      namespace: "api:products:feature",
      maxRequests: 10,
      windowSeconds: 3600,
    });
      ...
      return NextResponse.json(
        {
          ...
          message: "Has excedido el límite de productos destacados. Intenta más tarde.",
    ...
    if (error) {
      logger.error("sp_feature_product failed", {
        error: error.message,

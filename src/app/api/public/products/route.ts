...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (sortBy === "random") {
      ...
      if (rndError) {
        logger.error("Supabase random query failed", { error: rndError.message });
    ...
    if (error) {
      logger.error("Supabase products query failed", { error: error.message });
    ...
    logger.error("GET /api/public/products failed", { error: e?.message });

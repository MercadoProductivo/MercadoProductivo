...
export async function POST(_req: Request, ctx: { params: Promise<{ seller_id: string }> }) {
  try {
    ...
    // Asegurar que el perfil del usuario que da like exista para no violar FK
    {
      ...
      if (ensureErr) {
        logger.error("Failed to ensure liker profile", { user_id: user.id, error: ensureErr.message });
    ...
    if (likedRows && likedRows.length > 0) {
      ...
      const { error: delErr } = await supabase
        ...
        .eq("target_seller_id", sellerId);
      if (delErr) {
        logger.error("Failed to delete like", { seller_id: sellerId, user_id: user.id, error: delErr.message });
      ...
      if (insErr) {
        logger.error("Failed to insert like", { seller_id: sellerId, user_id: user.id, error: insErr.message });

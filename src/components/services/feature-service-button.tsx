...
export default function FeatureServiceButton({
  serviceId,
  defaultDays = 3,
  cost = 10,
  featuredUntil,
  ...
  const onConfirm = async () => {
    ...
    setError(null);
    try {
      const perDay = (featureCost ?? cost) || 0;
      const res = await fetch("/api/services/feature", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ serviceId, days: selectedDays, cost: perDay }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data?.error || "No se pudo destacar el servicio");
      }
      setOpen(false);
      router.refresh();
    } catch (e: any) {
      ...
      <Dialog open={openUnfeature} onOpenChange={setOpenUnfeature}>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              El servicio dejará de aparecer en la sección de destacados.
            </DialogDescription>
          </DialogHeader>
          {error && (
  ...
  if (!canFeature) {
    return (
      <Button size="sm" variant="outline" disabled title="Tu plan no permite destacar servicios">
        Destacar
      </Button>
    );
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Destacar servicio</DialogTitle>
          <DialogDescription>

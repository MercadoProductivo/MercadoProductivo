...
export default function MessagesInboxV2({ userId }: { userId: string }) {
    ...
    try {
      ...
      if (missingProfiles.length > 0) {
        try {
          ...
          logger.error('Error loading profiles in chat inbox', { error: String(error) });
        }
      }
  ...
  }, [setUnreadCount, chatConversationId, readPending]);
  ...
  useEffect(() => {
    ...
    let ch: PusherChannel | null = null;
    let retryTimer: ReturnType<typeof setTimeout> | null = null;
    let attempt = 0;
    let handlersBound = false;

    // Handlers
    const onConvUpdated = (raw: unknown) => {
      ...
      setItemsAll((prev) => {
        ...
        const updated = prev.map((conv) => {
          ...
          return {
            ...conv,
            // Importante: NO desocultar automáticamente al recibir un mensaje.
            // Si la conversación estaba oculta, debe permanecer en la pestaña "Ocultas".
    ...
    const onConvStarted = async (raw: unknown) => {
      const parsed = ConvUpdateEventSchema.safeParse(raw);
      if (!parsed.success) {
        logger.warn("Payload inválido chat:conversation:started", { error: parsed.error });
      }
      await refresh();
    };
    const onConvHidden = async (raw: unknown) => {
      const parsed = ConvUpdateEventSchema.safeParse(raw);
      if (!parsed.success) {
        logger.warn("Payload inválido chat:conversation:hidden", { error: parsed.error });
      }
      await refresh();
    };
    const onConvRestored = async (raw: unknown) => {
      const parsed = ConvUpdateEventSchema.safeParse(raw);
      if (!parsed.success) {
        logger.warn("Payload inválido chat:conversation:restored", { error: parsed.error });
      }
      await refresh();
    };

    const onRead = (raw: unknown) => {
      const parsed = ConvReadEventSchema.safeParse(raw);
      if (!parsed.success) {
        logger.warn("Payload inválido chat:conversation:read", { error: parsed.error });
        return;
      }
    ...
    const scheduleRetry = (status?: number) => {
      if (disposed) return;
      if (status === 403) {
        logger.warn("Pusher subscription forbidden (403)", { channel: channelName });
        return;
      }
      attempt += 1;
      const base = 500;
    ...
    const trySubscribe = () => {
      ...
      ch = subscribePrivate(channelName, {
        ...
        onSubscriptionError: (status) => { logger.warn("Pusher subscription error", { status, channel: channelName }); scheduleRetry(status); },
      });

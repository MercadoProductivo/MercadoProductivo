...
// íconos específicos ya vienen desde la config; no se requieren aquí
...
export default function GlobalMobileMenu({ initialIsSeller }: { initialIsSeller?: boolean }) {
  ...
  const accountItems = getDashboardNav(isSeller);
  ...
  return (
    <div className="fixed-safe-br z-50 lg:hidden flex flex-col items-end gap-3">
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent
          side="right"
          className="w-full max-w-[320px] sm:max-w-[400px] h-[100dvh] p-0 flex flex-col"
          onOpenAutoFocus={(e) => e.preventDefault()}
          onCloseAutoFocus={(e) => e.preventDefault()}
        >
          <SheetHeader className="pb-4 shrink-0 px-4 pt-4">
            <SheetTitle>
              {user ? (
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid gap-2">
                    ...
                    {/* CTA para convertirse en vendedor, solo si el usuario no es seller */}
                    {user && !isSeller && (
                      <Link
                        href="/ser-vendedor"
                        onClick={() => setOpen(false)}
                        className={cn(
                          "mt-1 inline-flex items-center justify-center rounded-md bg-orange-500 px-3 py-2 text-sm font-medium text-white hover:bg-orange-600"
                        )}
                      >
                        Ser vendedor
                      </Link>

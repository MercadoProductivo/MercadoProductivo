...
import { subscribePrivate, getPusherClient, onConnectionStateChange, ensurePusherFeatureReady } from "@/lib/pusher/client";
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (cachedData) {
        try {
          ...
          if (Date.now() - (timestamp || 0) < 30000) { // 30 segundos de caché
    ...
    async function fetchMessages() {
      try {
        ...
        // En cualquier caso (incluido AbortError), liberar loading para no dejar la UI bloqueada
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const bindHandlers = () => {
      ...
      const onChatMessageNew = async (msg: any) => {
        ...
        // Marcar como visto ANTES de modificar el estado para prevenir condiciones de carrera
        // en las que múltiples binds/llamadas simultáneas pasen el guard.
